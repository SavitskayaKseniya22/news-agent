...
export default function StoryPlaceholder({ length, type }: { length: number; type: StoryPreviewType }) {
  if (type === StoryPreviewType.GIGANTIC) {
    return Array.from({ length }, (_, index) => (
      <div className="flex h-112 animate-pulse gap-8 overflow-hidden" key={index}>
        <div className="bg-palette-gray-dark h-full w-[30%] shrink-0" />
  ...
  if (type === StoryPreviewType.MEDIUM) {
    return Array.from({ length }, (_, index) => (
      <li className="flex w-full animate-pulse flex-col gap-2 border-2" key={index}>
  ...
  if (type === StoryPreviewType.SMALL) {
    return Array.from({ length }, (_, index) => (
      <li className="flex min-h-24 shrink-0 animate-pulse gap-4" key={index}>
